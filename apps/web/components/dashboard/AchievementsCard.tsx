    ...
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mb-4 p-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg">
        <div className="flex items-center justify-between">
          <span className="text-sm text-amber-700">Total Points</span>
          <span className="text-xl font-bold text-amber-600">{totalPoints}</span>
        </div>
      </div>
...
function AchievementBadge({ achievement }: { achievement: Achievement }) {
  ...
  return (
    <div
      className={`relative group flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${
        unlocked
    ...
    >
      {/* Icon */}
      <span
        className={`text-2xl ${unlocked ? '' : 'grayscale opacity-50'}`}
      >
        {icon}
      </span>
